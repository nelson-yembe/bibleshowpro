...
import { Download, Star, Trash2, Upload, CheckCircle2, CloudDownload, FileUp } from "lucide-react";
...
export function BibleVersionsPanel({ embedded = false }: { embedded?: boolean } = {}) {
  const {
    catalog,
    importing,
    importProgress,
  ...
  return (
    <div className={embedded ? "" : "panel p-5"}>
      ...
      <div className={cn("space-y-3", (message || error || importing || importProgress) && "mt-3")}>
        {installed.length > 0 && (
          <CollapsibleSection
            dense
            id="bible-installed"
            defaultOpen
            title="Installed"
            description="Active translations on this device"
            icon={<CheckCircle2 className="h-4 w-4" />}
            headerRight={<StatusBadge variant="draft">{installed.length}</StatusBadge>}
          >
        ...
        <CollapsibleSection
          ...
          description="Free public-domain translations"
          icon={<CloudDownload className="h-4 w-4" />}
          headerRight={
            availableDownload.length > 0 ? (
              <StatusBadge variant="draft">{availableDownload.length}</StatusBadge>
            ) : null
          }
        >
          <button
        ...
        <CollapsibleSection
          dense
          id="bible-import"
          title="Import licensed translation"
          description="Bring your own licensed JSON (ESV, NIV, etc.)"
